...
const App = () => {
    ...
    const [weddingData, setWeddingData] = useState({
        ...
        vendors: [],
        florals: { bouquet: [] },
        seatingPlan: { 'Table 1': [], 'Table 2': [], 'Table 3': [], 'Table 4': [] },
    });
    ...
    useEffect(() => {
        ...
        const modes = {
            planning: { collection: 'weddings', setData: setWeddingData, initialData: { weddingDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toISOString().split('T')[0], guestList: [], budget: { estimated: 10000, expenses: [] }, todoList: [], venues: [], vendors: [], florals: { bouquet: [] }, seatingPlan: { 'Table 1': [], 'Table 2': [], 'Table 3': [], 'Table 4': [] } } },
            proposal: { collection: 'proposals', setData: setProposalPlan, initialData: { todoList: [{ id: 1, text: 'Research rings', completed: false }] } },
            simulator: { collection: 'simulations', setData: setDreamWedding, initialData: { theme: 'Modern', savedPalettes: [] } }
        };
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                setDoc(docRef, currentMode.initialData).catch(error => console.error("Error creating initial document:", error));
            }
        }, (error) => {
    ...
    const handleDataUpdate = async (newData, mode = appMode, options = { merge: true }) => {
        ...
        try {
            ...
            // This is where errors are caught. We need to see what this says.
    ...
    // CORRECTED: This function now preserves existing data and merges simulator data.
    const startWeddingPlanning = async (dreamData = null) => {
        // Start with the CURRENT weddingData state to preserve any existing data.
        let updatedWeddingData = { ...weddingData };

        // If coming from the simulator, integrate its suggestions.
        if (dreamData) {
            // Update the budget estimate without erasing existing expenses.
            updatedWeddingData.budget = {
                ...updatedWeddingData.budget,
                estimated: dreamData.budget
            };

            // Add new to-do items from the simulator without removing existing ones.
            ...
            // Prepend new tasks to the existing list.
            updatedWeddingData.todoList = [budgetTodo, themeTodo, ...(updatedWeddingData.todoList || [])];
        }

        // Perform a non-destructive merge. This is the safest way to update.
        // It will update fields but won't delete data you don't specify.
        await handleDataUpdate(updatedWeddingData, 'planning', { merge: true });
        setAppMode('planning');
    };
    ...
    const handleGenerateTasks = async () => {
        ...
        try {
            ...
            } else { throw new Error("AI returned no text."); }
        } catch (error) {
    ...
    const handleFloralsUpdate = (newFlorals) => {
        const bouquet = newFlorals?.bouquet || [];
        const bouquetTotalCost = bouquet.reduce((total, item) => total + (item.price * item.quantity), 0);
        const bouquetExpenseId = 'bridal-bouquet-expense';
        const bouquetExpenseItem = { id: bouquetExpenseId, item: 'Bridal Bouquet', category: 'Florals', estimated: bouquetTotalCost, actual: bouquetTotalCost, vendor: 'Flower Market' };
        const existingExpenses = weddingData.budget?.expenses || [];
        const expenseExists = existingExpenses.some(ex => ex.id === bouquetExpenseId);
        let updatedExpenses;
        if (bouquetTotalCost > 0) {
            if (expenseExists) {
                updatedExpenses = existingExpenses.map(ex => ex.id === bouquetExpenseId ? bouquetExpenseItem : ex);
            } else {
                updatedExpenses = [...existingExpenses, bouquetExpenseItem];
            }
        } else {
            updatedExpenses = existingExpenses.filter(ex => ex.id !== bouquetExpenseId);
        }
        const updatedData = { ...weddingData, florals: newFlorals, budget: { ...weddingData.budget, expenses: updatedExpenses } };
        setWeddingData(updatedData);
        handleDataUpdate(updatedData, 'planning');
    };

    const handleSeatingPlanUpdate = (newPlan) => {
        const updatedData = { ...weddingData, seatingPlan: newPlan };
        setWeddingData(updatedData);
        handleDataUpdate(updatedData, 'planning');
    };
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'seating': return <SeatingChart guests={weddingData.guestList || []} plan={weddingData.seatingPlan} onUpdate={handleSeatingPlanUpdate} />;
...
// ... The rest of the file (all the stateless components) is unchanged...
...
const AIVenueScout = ({ onSaveVenue, cache, setCache }) => { const { query, results, isLoading, error } = cache; const handleQueryChange = (field, value) => { setCache(prev => ({ ...prev, query: { ...prev.query, [field]: value } })); }; const handleSearch = async (e) => { e.preventDefault(); if (!query.location || !query.budget) { setCache(prev => ({ ...prev, error: 'Please enter a location and budget.' })); return; } setCache(prev => ({ ...prev, isLoading: true, error: null, results: [] })); const prompt = `Act as a wedding venue scout. Find 5 potential wedding venues in or near "${query.location}" with an estimated price for a wedding under $${query.budget}. For each venue, provide its name, location, an "aesthetic_description", an "estimated_price" as a number, its official "website_url", and a publicly accessible, direct hotlink to a high-quality image (must end in .jpg, .png, or .webp) under the key "image_url". Do not provide a URL to a webpage or search result. IMPORTANT: Respond with only a valid JSON object. The JSON object should have a single key "venues" which is an array of the venue objects. Do not include any text, titles, markdown formatting, or explanations before or after the JSON object.`; const payload = { prompt }; try { const response = await fetch('/.netlify/functions/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) { throw new Error(`API call failed`); } const result = await response.json(); const rawText = result.candidates?.[0]?.content?.parts?.[0]?.text; if (rawText) { const jsonString = rawText.substring(rawText.indexOf('{'), rawText.lastIndexOf('}') + 1); const parsedResult = JSON.parse(jsonString); setCache(prev => ({ ...prev, results: parsedResult.venues || [] })); } else { setCache(prev => ({ ...prev, error: "Could not find any venues." })); } } catch (err) { console.error("Error finding venues:", err); setCache(prev => ({ ...prev, error: `An error occurred: ${err.message}` })); } finally { setCache(prev => ({ ...prev, isLoading: false })); } }; const ensureProtocol = (url) => { if (!url) return '#'; if (!/^https?:\/\//i.test(url)) { return `https://${url}`; } return url; }; return ( <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md"> <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">AI Venue Scout 🤖</h1> <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4 mb-8"> <input type="text" value={query.location} onChange={e => handleQueryChange('location', e.target.value)} placeholder="Enter City or Area" className="flex-grow p-2 border rounded-md" /> <input type="number" value={query.budget} onChange={e => handleQueryChange('budget', e.target.value)} placeholder="Max Venue Budget" className="w-full md:w-1/4 p-2 border rounded-md" /> <button type="submit" disabled={isLoading} className="bg-pink-500 text-white px-6 py-2 rounded-md hover:bg-pink-600"> {isLoading ? 'Searching...' : 'Find Venues'} </button> </form> {error && <div className="text-red-500">{error}</div>} <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{results.map((venue, index) => ( <div key={index} className="border rounded-lg"> <img src={venue.image_url} alt={venue.name} className="w-full h-48 object-cover"/> <div className="p-4"> <h3 className="font-bold">{venue.name}</h3> <p>{venue.location}</p> <a href={ensureProtocol(venue.website_url)} target="_blank" rel="noopener noreferrer">Website</a> <button onClick={() => onSaveVenue(venue)}>Save</button> </div> </div> ))}</div> </div> ); };
...
const BudgetGauge = ({ currentValue, maxValue }) => { const percentage = Math.min((currentValue / maxValue) * 100, 100); let barColor = 'bg-green-500'; if (percentage > 75) barColor = 'bg-yellow-500'; if (percentage > 95) barColor = 'bg-red-500'; return ( <div className="mt-4"> <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4"> <div className={`${barColor} h-4 rounded-full transition-all duration-500`} style={{ width: `${percentage}%` }}></div> </div> </div> ); };
const SeatingChart = ({ guests, plan, onUpdate }) => {
    const safePlan = plan || { 'Table 1': [], 'Table 2': [], 'Table 3': [], 'Table 4': [] };
    const seatedGuestIds = new Set(Object.values(safePlan).flat());
    ...
    return (
        <div className="flex flex-col md:flex-row h-full gap-6">
            ...
            >
                ...
                <div className="space-y-2 h-full overflow-y-auto">
                    {unseatedGuests.map(guest => (
                        <div key={guest.id} draggable onDragStart={(e) => handleDragStart(e, guest.id)} className="p-3 bg-gray-100 dark:bg-gray-700 rounded-md cursor-grab active:cursor-grabbing">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 h-full overflow-y-auto">
                {Object.keys(safePlan).sort().map(tableName => {
                    const guestIds = safePlan[tableName];
                    return (
                        <div key={tableName} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, tableName)} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border-2 border-dashed border-gray-300 dark:border-gray-600">
                            ...
                            <div className="space-y-2">
                                {guestIds.map(id => {
                                    ...
                                    return (
                                        <div key={guest.id} draggable onDragStart={(e) => handleDragStart(e, guest.id)} className="p-2 bg-pink-100 dark:bg-pink-900/50 rounded-md cursor-grab active:cursor-grabbing">
